...
exports.signup = (req, res) => {
  ...
  User.create({
    ...
    postcode: req.body.profileData.postcode,
    address1: req.body.profileData.address1,
    address2: req.body.profileData.address2
  }).then(user => {
    if (req.body.roles) {
      Role.findAll({
        where: {
...
exports.signin = (req, res) => {
  User.findOne({
    ...
    var token = jwt.sign({ id: user.id }, config.secret, { expiresIn: SECONDS_IN_DAY });
    var authorities = [];
